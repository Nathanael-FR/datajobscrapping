...
class HelloWorkScrapper(Scrapper):
    ...
    def get_max_pages(html: HTMLParser) -> int:
        ...
        try:
            navbar = html.css_first("nav.tw-flex.tw-gap-2.tw-typo-m.tw-flex-wrap")
    ...
    def parse_job_offer(self, job_offer_url: str) -> JobItem:
        ...
        company_name = self.extract_text(html, "span.tw-contents.tw-typo-m.tw-text-grey")
        salary = self.extract_text(html, "li.tw-tag-attractive-s.tw-readonly") if html.css_first("li.tw-tag-attractive-s.tw-readonly") else None
        remote = self.extract_text(html, "li.tw-tag-primary-s.tw-readonly") if html.css_first("li.tw-tag-primary-s.tw-readonly") else None
        
        if remote and "Télétravail" not in remote:
            remote = None

        job_tags = html.css("li.tw-block.tw-tag-primary-s.tw-readonly.tw-w-fit.tw-whitespace-nowrap.tw-text-ellipsis.tw-overflow-hidden")
